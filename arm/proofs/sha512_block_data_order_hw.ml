...
(* Load instruction's effects *)

g`forall pc retpc sp input_base input.
PAIRWISE nonoverlapping
  [(word sp:int64, 128); (word pc:int64, LENGTH sha512_hw_mc);
   (word input_base:int64, 16 * 8)]
==>
ensures arm
  // Precondition
  (\s. aligned_bytes_loaded s (word pc) sha512_hw_mc /\
       ...
       read (memory :> bytes128 (word input_base)) s = input
       // (???) How can I use bignum_from_memory here and get 
       // the conclusion marked by (???)?
       // bignum_from_memory (word input_base, 16) s = input
       )
  // Postcondition
  (\s. 
  // Desired conclusion: (???)
  // read Q16 s = word_subword ((word input) : 1024 word) (0, 128)
  read Q16 s = input
  )
...
e(ARM_STEPS_TAC SHA512_HW_EXEC (1--3));;
...
let sha512su1_sha512su0_at_zeroth_argument = prove(
 `!(a0:(64)word) (a1:(64)word) (b0:(64)word) (b1:(64)word) 
   (c0:(64)word) (c1:(64)word) (d0:(64)word) (d1:(64)word).
    ...
    // (TODO): Abstract away (a1 + sigma0 b0) + sigma1 c1 + d1 into a function used in
    // sha512_sched.
    word_join ((a1 + sigma0 b0) + sigma1 c1 + d1)
              ((a0 + sigma0 a1) + sigma1 c0 + d0) :(128)word`,
REWRITE_TAC[sha512su0; sha512su1] THEN
CONV_TAC(TOP_DEPTH_CONV let_CONV) THEN
REWRITE_TAC[WORD_BITMANIP_SIMP_LEMMAS; REV64_BITMANIP_SIMP_LEMMAS]);;
...
ensures arm
  ...
  (\s. aligned_bytes_loaded s (word pc) sha512_hw_mc /\
       ...
       read (memory :> bytes128 (word ktbl_base + word 16))     s = word_join  (word K_3 : 64 word)  (word K_2 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*2))) s = word_join  (word K_5 : 64 word)  (word K_4 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*3))) s = word_join  (word K_7 : 64 word)  (word K_6 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*4))) s = word_join  (word K_9 : 64 word)  (word K_8 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*5))) s = word_join (word K_11 : 64 word) (word K_10 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*6))) s = word_join (word K_13 : 64 word) (word K_12 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*7))) s = word_join (word K_15 : 64 word) (word K_14 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*8))) s = word_join (word K_17 : 64 word) (word K_16 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*9))) s = word_join (word K_19 : 64 word) (word K_18 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*10))) s = word_join (word K_21 : 64 word) (word K_20 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*11))) s = word_join (word K_23 : 64 word) (word K_22 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*12))) s = word_join (word K_25 : 64 word) (word K_24 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*13))) s = word_join (word K_27 : 64 word) (word K_26 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*14))) s = word_join (word K_29 : 64 word) (word K_28 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*15))) s = word_join (word K_31 : 64 word) (word K_30 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*16))) s = word_join (word K_33 : 64 word) (word K_32 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*17))) s = word_join (word K_35 : 64 word) (word K_34 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*18))) s = word_join (word K_37 : 64 word) (word K_36 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*19))) s = word_join (word K_39 : 64 word) (word K_38 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*20))) s = word_join (word K_41 : 64 word) (word K_40 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*21))) s = word_join (word K_43 : 64 word) (word K_42 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*22))) s = word_join (word K_45 : 64 word) (word K_44 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*23))) s = word_join (word K_47 : 64 word) (word K_46 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*24))) s = word_join (word K_49 : 64 word) (word K_48 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*25))) s = word_join (word K_51 : 64 word) (word K_50 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*26))) s = word_join (word K_53 : 64 word) (word K_52 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*27))) s = word_join (word K_55 : 64 word) (word K_54 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*28))) s = word_join (word K_57 : 64 word) (word K_56 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*29))) s = word_join (word K_59 : 64 word) (word K_58 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*30))) s = word_join (word K_61 : 64 word) (word K_60 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*31))) s = word_join (word K_63 : 64 word) (word K_62 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*32))) s = word_join (word K_65 : 64 word) (word K_64 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*33))) s = word_join (word K_67 : 64 word) (word K_66 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*34))) s = word_join (word K_69 : 64 word) (word K_68 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*35))) s = word_join (word K_71 : 64 word) (word K_70 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*36))) s = word_join (word K_73 : 64 word) (word K_72 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*37))) s = word_join (word K_75 : 64 word) (word K_74 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*38))) s = word_join (word K_77 : 64 word) (word K_76 : 64 word) /\ 
       read (memory :> bytes128 (word ktbl_base + word (16*39))) s = word_join (word K_79 : 64 word) (word K_78 : 64 word) /\ 
       ...
       read (memory :> bytes128 (word input_base + word 16)) s =  word_join (word_bytereverse (i3 : 64 word))   (word_bytereverse (i2 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 32)) s =  word_join (word_bytereverse (i5 : 64 word))   (word_bytereverse (i4 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 48)) s =  word_join (word_bytereverse (i7 : 64 word))   (word_bytereverse (i6 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 64)) s =  word_join (word_bytereverse (i9 : 64 word))   (word_bytereverse (i8 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 80)) s =  word_join (word_bytereverse (i11 : 64 word))  (word_bytereverse (i10 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 96)) s =  word_join (word_bytereverse (i13 : 64 word))  (word_bytereverse (i12 : 64 word)) /\
       read (memory :> bytes128 (word input_base + word 112)) s = word_join (word_bytereverse (i15 : 64 word))  (word_bytereverse (i14 : 64 word)) /\
       ...
       read (memory :> bytes128 (word hash_base + word 16)) s = word_join (word h_d : 64 word) (word h_c : 64 word) /\
       read (memory :> bytes128 (word hash_base + word 32)) s = word_join (word h_f : 64 word) (word h_e : 64 word) /\
       read (memory :> bytes128 (word hash_base + word 48)) s = word_join (word h_h : 64 word) (word h_g : 64 word)
       // (a, b, c, d, e, f, g, h) = 
...
(* Simulate the REV64 instructions. *)
e(ARM_STEPS_TAC SHA512_HW_EXEC (15--22));;
(* Simplify REV64 semantics. *)
(* (TODO) Speed up by simplifing after each REV64 instruction. *)
...
e(RULE_ASSUM_TAC(REWRITE_RULE[WORD_BITMANIP_SIMP_LEMMAS]));;
(* 0x3cc10479;  ldr	   q25, [x3], #16  *)
e(ARM_STEPS_TAC SHA512_HW_EXEC (26--26));; 
e(ARM_STEPS_TAC SHA512_HW_EXEC (27--33));; (* until SHA512H *)
e(RULE_ASSUM_TAC(REWRITE_RULE[WORD_BITMANIP_SIMP_LEMMAS]));;
e(ARM_STEPS_TAC SHA512_HW_EXEC (34--37));; 
e(RULE_ASSUM_TAC(REWRITE_RULE[WORD_BITMANIP_SIMP_LEMMAS]));;
(* 
 Simplifying
  sha512su1 (sha512su0 (word_join i1 i0) (word_join i3 i2))
            (word_join i15 i14)
            (word_join i10 i9)
*)
e(RULE_ASSUM_TAC(REWRITE_RULE[sha512su1_sha512su0_at_zeroth_argument]));;
